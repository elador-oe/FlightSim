...
public class MapDrawer extends Canvas {
	...
	public Point setRoute(double clickedX, double clickedY) { // Setting destination
		...
		toDrawCol = (int) ((clickedX) / cellSize)-1;
		toDrawRow = (int) ((clickedY) / cellSize)+6;
		redraw();
	...
	public void redraw() {
		if (heightMatrix != null) {
			double W = getWidth();
			double H = getHeight();
			double w = W / heightMatrix.length;
			...
			for (int i = 0; i < heightMatrix.length; i++) {
				for (int j = 0; j < heightMatrix[i].length; j++) {
					...
					if (i == toDrawRow && j == toDrawCol) {
							...
							gc.drawImage(destination, j * w, i * h, 25, 25); // Draw destination
					}
				}
			}

			for (Point p : points) {
				gc.drawImage(path, p.x * w, p.y * h, 25, 25);
			}
			gc.drawImage(airplane, aCol * w, aRow * h ,30,30);
		}
